...
for t in tasks:
	...
	for s in subjects:
		#try:		
			...
			iFile = csv.reader(open(fileLocation + t + "_"+str(s)+"_2_L" + str(lasts[lastsCount]) + "_Z/" + 'stats.csv','r'))
			...
			iFile = csv.reader(open(fileLocation +  t + "_"+str(s)+"_2_L" + str(lasts[lastsCount]) + "_Z/"+ str(bestInd) + '_multi.txt','r'), delimiter='\t')				#CHANGE HERE
			for line in iFile:
				if(line[-1][0] == 'v'):
					eInt = int(line[-1][1:])
					fcount[eInt] = 1
